...
async def generate(interaction: discord.Interaction, ilosc: int = 1):
    if not is_owner(interaction):
        await interaction.response.send_message("❌ Nie masz uprawnień.")
        return

    if ilosc < 1 or ilosc > 20:
        await interaction.response.send_message("❌ Możesz wygenerować od 1 do 20 kluczy naraz.")
        return
...
async def list_keys(interaction: discord.Interaction):
    ...
    keys = load_keys()
    if not keys:
        await interaction.response.send_message("Brak kluczy.")
        return
...
async def revoke(interaction: discord.Interaction, klucz: str):
    ...
    if klucz not in keys:
        await interaction.response.send_message("❌ Taki klucz nie istnieje.")
        return

    del keys[klucz]
    save_keys(keys)
    await interaction.response.send_message(f"✅ Klucz `{klucz}` został usunięty.")
